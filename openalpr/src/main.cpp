...
int main( int argc, const char** argv )
{
  ...
  TCLAP::UnlabeledMultiArg<std::string>  fileArg( "image_file", "Image containing license plates colombian version", true, "", "image_file_path"  );

  
  TCLAP::ValueArg<std::string> countryCodeArg("c","country","Country code to identify (default co for Colombia  or eu for Europe).  Default=co",false, "co" ,"country_code");
  ...
  for (unsigned int i = 0; i < filenames.size(); i++)
  {
    ...
    } //En esta parte verifica si el comando es webcam para arrancarla default number0
...
bool detectandshow( Alpr* alpr, cv::Mat frame, std::string region, bool writeJson)
{
  ...
  if (writeJson)
  {
    // En esta parte se crea el metodo que se desea se responda para cuando se invoque la camara
  ...
  else
  {
    ...
    {
      std::cout << "placa" << i << ": " << results.plates[i].topNPlates.size() << " resultado";
      if (measureProcessingTime)
        std::cout << " -- Tiempo Procesamiento = " << results.plates[i].processing_time_ms << "ms.";
      std::cout << std::endl;
      ...
      for (int k = 0; k < results.plates[i].topNPlates.size(); k++)
      {
        ...
        std::cout << "    - " << no_newline << "\t probabilidad: " << results.plates[i].topNPlates[k].overall_confidence;
